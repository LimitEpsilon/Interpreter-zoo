...
with shdw :=
  ...
  | Call (s : shdw) (v : vl)
  | SuccS (s : shdw) (* new! *)
  | CaseS (s : shdw) (vO : option vl) (x : var) (kS : option vl) (σS : nv) (* new! *)
.
...
Section IND.
  ...
  Context (PInit : Pshdw Init)
          ...
          (PCaseS : forall s vO x kS σS, Pshdw s ->
            match vO with None => True | Some vO => Pvl vO end ->
            Pnv σS ->
            Pshdw (CaseS s vO x kS σS)).
  ...
  with shdw_ind s :=
    ...
    | CaseS s vO x kS σS =>
      PCaseS s vO x kS σS (shdw_ind s)
...
Local Notation "'⟪' s '|' 'O' '⤇' vO '|' 'S' '⤇' 'λ' x kS σS '⟫'" := (CaseS s vO x kS σS) (at level 60, right associativity, only printing).
...
with alloc_shdw (s : shdw) :=
  ...
  | CaseS s vO x kS σS =>
    ...
    Pos.max (alloc_shdw s) (Pos.max (lift_alloc vO) (alloc_nv σS))
  end.
...
with size_shdw (s : shdw) :=
  ...
  | CaseS s vO x kS σS =>
    ...
    S (Nat.max (size_shdw s) (Nat.max (lift_size vO) (size_nv σS)))
  end.
  ...
  | Case E M x N =>
    let foldE e :=
      ...
      | vl_sh s =>
        let vO := eval M in
        let kS := eval N in
        Some (vl_sh (CaseS s vO x kS (nv_sh Init)))
      | vl_exp _ | vl_clos _ _ _ => None
      end
    ...
    | CaseS s vO x kS σS =>
      ...
      let folds s :=
        let foldσ σ :=
          ...
          | vl_nat O => vO
          | vl_nat (S m) =>
            lift (link n (nv_bval x (wvl_v (vl_nat m)) σ)) kS
          | vl_sh s => Some (vl_sh (CaseS s vO x kS σ))
          | vl_exp _ | vl_clos _ _ _ => None
          end
        in lift foldσ (link_nv σS (Acc_inv ACC _))
      in lift folds (link_shdw s (Acc_inv ACC _))
    end eq_refl
  for link_vl
...
Compute interp 7 three_plus_three.
Compute interp 7 x_plus_three.
...
Compute interp 1 sum_tm.

Definition unknown_function :=
  App (App sum_tm (Var "f")) three_tm.

Compute interp 7 unknown_function.
...
Compute sem_link 7 (Some (nv_bval "n" (wvl_v (vl_nat 3)) (nv_sh Init)))
